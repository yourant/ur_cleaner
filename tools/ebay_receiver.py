...
class EbayHandler(BaseService):
    ...
    def get_trades(self):
        sql = ("select pt.nid,ptd.l_number,'p_trade' as tableName from p_trade(nolock) as pt "
               ...
               "and convert(varchar(7),closingdate,121)='2019-04' union "
               ...
               "where addressowner='ebay' and isnull(receiverbusiness,'') = '' "
               "and convert(varchar(7),closingdate,121)='2019-04'")
    ...
    def set_paypal(self, row):
        sql = 'update {} set receiverbusiness=%s, receiveremail=%s where nid=%s'
        try:
            self.cur.execute(sql.format(row['tableName']), (row['payPal'], row['payPal'], row['nid']))
            self.con.commit()
