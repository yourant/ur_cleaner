...
class DevRateGoodsFetcher(CommonService):
    ...
    def fetch(self, date_flag, begin_date, end_date):
        ...
        for row in ret:
            yield (
                row['orderTime'], row['dateFlag'], row['SalerName'], row['goodsCode'], row['SaleMoneyRmbUS'],
    ...
    def push(self, rows):
        sql = ['insert into cache_devRateGoodsProfit(',
               'ordertime , dateFlag , salerName, goodsCode, saleMoneyRmbUS , saleMoneyRmbZn, costMoneyRmb , PPebayUS ,'
